...
// MCP tool handlers for URL co-sharing cluster investigation
...
function buildClustersQuery(params: {
  ...
  const { did, cluster_id, date, min_members, limit } = params;
...
export async function registerCosharingTools(
  ...
  server.tool(
    "cosharing_pairs",
    "Get raw co-sharing pairs for a specific account — which other accounts share the same URLs on the same day. Returns paired accounts, edge weights (number of co-shared URLs), and the actual shared URLs. Useful for drilling into a specific account's co-sharing relationships.",
    {
      did: z
        .string()
  ...
  server.tool(
    "cosharing_evolution",
    "Trace the evolution history of a URL co-sharing cluster over time. Shows how a cluster was born, continued, merged, split, or died across days. Returns timeline of evolution events with membership changes and predecessor/successor relationships.",
    {
      cluster_id: z
        .string()
